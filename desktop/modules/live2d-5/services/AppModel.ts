...
export class AppModel extends CubismUserModel {
  ...
  private setupViewMatrix(canvasWidth: number, canvasHeight: number): void {
    ...
    // ★ 修正顺序：先 scale 再 translate
    // translateRelative 和 scaleRelative 都是左乘（this = new × this），
    // 所以先调 scale 在最右边（最先执行），后调 translate 在最左边（最后执行）
    // 变换链：P' = translate × scale × P → 先缩放到逻辑坐标，再平移到原点
  ...
  /**
   * 点击事件处理（使用逻辑坐标）
   * 与官方 Demo LAppLive2DManager.onTap 一致：仅使用 HitArea 检测
   */
